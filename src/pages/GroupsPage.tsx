...
export default function GroupsPage({ onNavigate }: Props) {
    const { groupStandings, thirdPlaceStandings, currentProfile } = useTournament();
    ...
    const handleDownloadSummary = async () => {
        ...
        try {
            ...
            link.download = `groups-stage-predictions-summary.png`;
            link.href = dataUrl;
            link.click();
        } catch (error) {
            console.error("Failed to generate groups summary image", error);
        } finally {
            setIsDownloading(false);
        }
    };
    ...
    return (
        <div className="space-y-10 animate-fadeIn">
            {/* Quick explanation and instructions (Hidden during download) */}
            {!isDownloading && (
                <div className="bg-slate-900/30 border border-slate-800/80 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 backdrop-blur-sm">
                    ...
                    <button
                        ...
                        className="flex-shrink-0 px-4 py-2.5 rounded-xl bg-blue-950/20 border border-blue-900/35 hover:bg-blue-950/40 text-blue-400 font-bold text-xs uppercase tracking-wider transition-all active:scale-95 flex items-center gap-1.5"
                    >
                        <Camera size={14} />
            ...
            {/* Printable summary content wrapper */}
            <div ref={summaryRef} className="space-y-10 bg-slate-950">
                        ...
                        {groups.map((group) => {
                            ...
                            return (
                                <GroupCard
                                    ...
                                    groupId={group.id}
                                    standings={standings}
                                    onClick={() => !isDownloading && onNavigate("group", group.id)}
                                />
                            );
                        })}
                    </div>
